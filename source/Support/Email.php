...
class Email
{
     ...
     * @param string $toNrecipientNameame
     * @return Email
     */
    public function bootstrap(string $subject, string $body, string $recipient, string $recipientName, ?array $data): Email
    {
    ...
    public function queue(string $from = CONF_MAIL_SENDER['address'], string $fromName = CONF_MAIL_SENDER["name"]): bool
    {
        try {
            $stmt = Connect::getInstance()->prepare(
                "INSERT INTO mail_queues (from_email, from_name, body, recipient_email, recipient_name, subject, ip_adress, browser_info, created_by) 
                 VALUES(:from_email, :from_name, :body, :recipient_email, :recipient_name, :subject, :ip_adress, :browser_info, :created_by)"
            );
            ...
            $stmt->bindValue(":ip_adress", $this->data->ip_adress, \PDO::PARAM_STR);
            $stmt->bindValue(":browser_info", $this->data->browser_info, \PDO::PARAM_STR);
            $stmt->bindValue(":created_by", $this->data->created_by, \PDO::PARAM_INT);
            $stmt->execute();
            return true;
