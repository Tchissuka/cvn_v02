 ...
 *
 * @author Pedro J. Mariano <pejuma4@hotmail.com>
...
abstract class Model
{
     ...
     */
    public function join(string $query)
    {
        try {
    ...
    public function fetch(bool $all = false)
    {
        try {
            ...
            $stmt = $pdo->prepare($this->query . $this->group . $this->order . $this->limit . $this->offset);
    ...
    protected function create(array $data): ?int
    {
        try {
            ...
            $stmt = $pdo->prepare("INSERT INTO {$this->entity} ({$columns}) VALUES ({$values})");
    ...
    protected function update(array $data, string $terms, string $params): ?int
    {
        try {
            ...
            $stmt = $pdo->prepare("UPDATE {$this->entity} SET {$dateSet} WHERE {$terms}");
    ...
    public function delete(string $terms, ?string $params): bool
    {
        try {
            ...
            $stmt = $pdo->prepare("DELETE FROM {$this->entity} WHERE {$terms}");
            if ($params) {
                parse_str($params, $params);
                $stmt->execute($params);
                return true;
